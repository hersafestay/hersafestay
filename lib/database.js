...
export async function getPropertiesForCity(cityId) {
  // Step 1: fetch properties without join (avoids PostgREST FK relationship errors)
  ...
  if (error) {
    console.error('[getPropertiesForCity] properties error:', error);
    return { data: [], error };
  }
  ...
  // Step 2: fetch zone data for all unique zone IDs in one query
  const zoneIds = [...new Set(properties.map((p) => p.zone_id).filter(Boolean))];
  let zoneMap = {};

  if (zoneIds.length > 0) {
    const { data: zones, error: zoneError } = await supabase
      .from('safety_zones')
      .select('id, zone_name, safety_level, color_code')
      .in('id', zoneIds)
      .eq('is_published', true);

    if (zoneError) {
      console.error('[getPropertiesForCity] zones error:', zoneError);
      // continue without zone data rather than failing entirely
    } else {
      zoneMap = Object.fromEntries((zones || []).map((z) => [z.id, z]));
    }
  }

  // Step 3: attach zone to each property
  const data = properties.map((p) => ({ ...p, zone: zoneMap[p.zone_id] ?? null }));
