...
export function getPropertyMarkerIcon(propertyType, isSelected = false, isHovered = false) {
  ...
  // Selected: ~1.5x normal; hovered: slightly larger than normal
  const w = isSelected ? 40 : isHovered ? 30 : 26;
  const h = isSelected ? 52 : isHovered ? 39 : 34;

  // Selected: wider viewBox so 5 px white halo stroke doesn't get clipped at edges
  const viewBox = isSelected ? '-3 -3 34 42' : '0 0 28 36';
  ...
  const svg = [
    ...
    // paint-order="stroke fill" draws white stroke behind the fill → visible halo ring
    ` fill="${color}" stroke="white" stroke-width="${strokeWidth}"${isSelected ? ' paint-order="stroke fill"' : ''}/>`,
    `<circle cx="14" cy="14" r="5.5" fill="white" opacity="${isSelected ? 1 : 0.92}"/>`,
    `</svg>`,
  ].join('');
